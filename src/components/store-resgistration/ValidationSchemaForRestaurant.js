...
const ValidationSchemaForRestaurant = () => {
	...
	return Yup.object({
		...
		ine_image: Yup.mixed()
			.required(t("Please upload INE front"))
			.test(
				"fileSize",
				t("File size must be less than 2MB"),
				(value) => value && value.size <= IDENTITY_FILE_SIZE
			)
			.test(
				"fileFormat",
				t("Unsupported Format"),
		...
		ine_back_image: Yup.mixed()
			.required(t("Please upload INE back"))
			.test(
				"fileSize",
				t("File size must be less than 2MB"),
				(value) => value && value.size <= IDENTITY_FILE_SIZE
		...
		cofepris_document_image: Yup.mixed()
			.nullable()
			.test(
				"fileSize",
				t("File size must be less than 2MB"),
				(value) => !value || (value && value.size <= IDENTITY_FILE_SIZE)
			),
	});
};
